...
if (isset($_POST['video_completed'])) {
   // Assuming you have a way to identify the content being watched
   $content_id = $_POST['content_id'];
   $content_id = filter_var($content_id, FILTER_SANITIZE_STRING);

   // Check if there is already a record for this user and content
   $select_progress = $conn->prepare("SELECT * FROM `user_video_progress` WHERE user_id = ? AND content_id = ?");
   $select_progress->execute([$user_id, $content_id]);

   if ($select_progress->rowCount() > 0) {
      // Update progress if the record already exists
      $update_progress = $conn->prepare("UPDATE `user_video_progress` SET progress = 100 WHERE user_id = ? AND content_id = ?");
      $update_progress->execute([$user_id, $content_id]);
   } else {
      // Insert a new record if it doesn't exist
      $insert_progress = $conn->prepare("INSERT INTO `user_video_progress` (user_id, content_id, progress) VALUES (?, ?, 100)");
